...
AAA && BBB && CCC
```
image::basicsequence.png[basic sequence]

You can execute the same task multiple times in a sequence.  For example:
```
AAA && AAA && AAA
```
image::samejobsequence.png[basic sequence with repeated job definition]
...
```
image::basictransition.png[basic transition]
...
```
image::basictransitionwithsequence.png[basic transition with sequence]
...
AAA 'FAILED' -> BBB '*'->CCC
```
image::basictransitionwithwildcard.png[basic transition with wildcard]
...
<AAA || BBB || CCC>
```
image::basicsplit.png[basic split]
...
<AAA || BBB || CCC> && DDD && EEE
```
image::basicsplitwithsequence.png[basic split with sequence]
